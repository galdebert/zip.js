...
async function test() {
	...
	for (const cfg of cfgs) {
		...
		for (let i = 0; i < cfg.entryCount; i++) {
			...
			inputs.push({ name: `entry #${i + 1}`, data: getBlobOrU8(byteSize, cfg.inputType) });
		}
...
async function zip(cfg, inputs) {
	...
	await Promise.all(inputs.map(input => zipWriter.add(input.name, createReader(input.data), {})));
...
async function unzip(cfg, zipped) {
	...
	const proms = entries.map((entry) => entry.directory ? Promise.resolve(null) : entry.getData(createWriter(cfg.outputType)));
